"""
NOTE: [DEPERICIATED METHOD]
A basic slash command implementation for testing purposes.
"""
...
from pda.enums import (
    ApplicationCommandOptionType,
    ApplicationCommandPermissionType,
    InteractionResponseType,
)
from pda.flags import CallbackFlags
...
class PartialObject(pda.Object):
    """Subclasses of this have their .:attr:`guild` as a
    :class:`~pda.Object`, so their guild-related functionality may break.
    .. attribute:: guild
        :type: pda.Object
    """
...
class PartialTextChannel(pda.TextChannel, PartialObject):
    """A partial :class:`~pda.TextChannel`."""

    pass


class PartialCategoryChannel(pda.CategoryChannel, PartialObject):
    """A partial :class:`~pda.CategoryChannel`."""

    pass


class PartialVoiceChannel(pda.VoiceChannel, PartialObject):
    """A partial :class:`~pda.VoiceChannel`."""

    pass


class PartialMember(pda.Member, PartialObject):
    """A partial :class:`~pda.Member`."""

    pass


class PartialRole(pda.Role, PartialObject):
    """A partial :class:`~pda.Role`."""

    pass


class Context(pda.Object, _AsyncInit):
    """Object representing an interaction.
    .. attribute:: id
        :type: int
        The interaction ID.
    .. attribute:: guild
        :type: Union[pda.Guild, pda.Object]
        The guild where the interaction took place.
        Can be an :class:`~pda.Object` with just the ID
        if the client is not in the guild.
    .. attribute:: channel
        :type: Union[pda.TextChannel, pda.Object]
    ...
    .. attribute:: author
        ...
        If :attr:`guild` is an :class:`~pda.Object`, a lot of
        :class:`~pda.Member` methods that require the guild will break
    ...
    .. attribute:: me
        :type: Optional[pda.Member]
        The bot, as a :class:`~pda.Member` in that context.
        Can be :const:`None` if the client is not in the guild.
    .. attribute:: client
        :type: SlashBot
        The bot.
    .. attribute:: webhook
        :type: Optional[pda.Webhook]
        Webhook used for sending followup messages.
        :const:`None` until interaction response has been sent
    """

    id: int
    guild: Union[pda.Guild, pda.Object, None]
    channel: Union[pda.TextChannel, pda.Object]
    author: Union[pda.Member, PartialMember, None]
    command: Union[Command, Group]
    options: Mapping[str, Any]
    me: Union[pda.Member, pda.Object]
    client: SlashBot
    webhook: Optional[pda.Webhook]
    ...
    async def __init__(self, client: SlashBot, cmd: Command, event: dict):
        ...
        if event.get("guild_id", None):
            self.guild = await self._try_get(
                pda.Object(event["guild_id"]),
                self.client.get_guild,
                self.client.fetch_guild,
                "guild",
            )
        else:
            self.guild = None
        self.channel = await self._try_get(
            pda.Object(event["channel_id"]),
        ...
        if event.get("member", None):
            ...
            self.author = await self._try_get(
                author, self._get_member, self._fetch_member, "author-member"
            )
        else:
            self.author = None
        ...
        await self._kwargs_from_options(
            ...
            event["data"].get(
                "resolved", {"members": {}, "users": {}, "channels": {}, "roles": {}}
            ),
        )
        self.me = await self._try_get(
            pda.Object(self.client.user.id),
            self._get_member,
            self._fetch_member,
            "me-member",
        )
    ...
    async def _try_get(
        ...
        *,
        ...
        if fq or resolved is None or str(default.id) not in resolved[typename + "s"]:
            try:
                ...
                if obj is None and fng:
                    ...
                    logger.debug(
                        "Got %s %s for interaction %s", typename, obj.id, self.id
                    )
                    return obj
                ...
                logger.debug(
                    "Fetched %s %s for interaction %s", typename, obj.id, self.id
                )
                return obj
        if resolved is None:
            return default
        ...
        if obj is None:
            ...
            return default
        logger.debug("Resolved %s %s for interaction %s", typename, default.id, self.id)
        return resolve_method(obj)

    async def _try_get_user(
        self, value: pda.Object, resolved: dict, try_user: bool = True
    ):
        def resolve_member(member):
            member["user"] = resolved["users"][str(value.id)]
            return PartialMember(
                data=member, guild=self.guild, state=self.client._connection
            )

        def resolve_user(user):
            return pda.User(state=self.client._connection, data=user)
        ...
        value = await self._try_get(
            ...
            self._fetch_member,
            "member",
            resolve_method=resolve_member,
            resolved=resolved,
        )
        if type(value) is pda.Object and try_user:
            value = await self._try_get(
                value,
                None,
                None,
                "user",
                fq=False,
                resolve_method=resolve_user,
                resolved=resolved,
            )
        return value

    async def _try_get_channel(self, value: pda.Object, resolved: dict):
        def get_channel(oid):
            return self.guild.get_channel(oid)

        def resolve_channel(channel):
            # pda.py doesn't access this with a default,
            ...
            ctype = channel["type"]
            ctype, _ = pda.channel._channel_factory(ctype)
            if ctype is pda.TextChannel:
                ctype = PartialTextChannel
            elif ctype is pda.CategoryChannel:
                ctype = PartialCategoryChannel
            elif ctype is pda.VoiceChannel:
                ctype = PartialVoiceChannel
            return ctype(state=self.client._connection, guild=self.guild, data=channel)
        ...
        return await self._try_get(
            ...
            self.client.fetch_channel,
            "channel",
            resolve_method=resolve_channel,
            resolved=resolved,
        )

    async def _try_get_role(self, value: pda.Object, resolved: dict):
        def get_role(oid):
            return self.guild.get_role(oid)

        def resolve_role(role):
            # monkeypatch for pda.py
            role["permissions_new"] = role["permissions"]
            return PartialRole(
                state=self.client._connection, guild=self.guild, data=role
            )
    ...
    async def respond(
        ...
        content="",
        *,
        embed: pda.Embed = None,
        embeds: Iterable[pda.Embed] = None,
        allowed_mentions: pda.AllowedMentions = None,
        file: pda.File = None,
        ...
        :param pda.Embed embed: Shorthand for ``respond(embeds=[embed])``
        :param embeds: Up to 10 embeds (any more will be silently discarded)
        :type embeds: Iterable[pda.Embed]
        :param pda.AllowedMentions allowed_mentions:
            Mirrors normal ``allowed_mentions`` in
            :meth:`~pda.abc.Messageable.send`
        :param pda.File file:
            Mirrors normal ``file`` in :meth:`~pda.abc.Messageable.send`
        ...
        if self.webhook is not None:
            ...
            path = (
                f"/webhooks/{self.client.app_info.id}/{self.token}"
                "/messages/@original"
            )
            route = _Route(
                "PATCH", path, channel_id=self.channel.id, guild_id=self.guild.id
            )
        else:
            data = {"type": int(rtype)}
            ...
            route = _Route(
                "POST", path, channel_id=self.channel.id, guild_id=self.guild.id
            )
            self.webhook = pda.Webhook.partial(
                id=self.client.app_info.id,
                token=self.token,
                adapter=pda.AsyncWebhookAdapter(self.client.http._HTTPClient__session),
            )
        if isinstance(file, pda.File):
            form = []
            form.append({"name": "payload_json", "value": pda.utils.to_json(data)})
            form.append(
                {
                    "name": "file",
                    "value": file.fp,
    ...
    async def delete(self):
        ...
        path = f"/webhooks/{self.client.app_info.id}/{self.token}" "/messages/@original"
        route = _Route(
            "DELETE", path, channel_id=self.channel.id, guild_id=self.guild.id
        )
        await self.client.http.request(route)

    async def send(self, *args, **kwargs):
        """Send a message in the channel where the the command was run.
        Equivalent to :meth:`~pda.TextChannel.send`
...
class Option:
    ...
    def __repr__(self):
        return (
            "Option(name={0.name!r}, type='{0.type!s}', description=..., "
            "required={0.required}, choices={1})"
        ).format(self, "[...]" if self.choices else "[]")
...
CommandPermissionsDict = Dict[
    Optional[int], Dict[Tuple[int, ApplicationCommandPermissionType], bool]
]


class Command(pda.Object):
    ...
    def __init__(self, coro: Coroutine, **kwargs):
        ...
        for param in signature(coro).parameters.values():
            ...
            if (
                not (
                    isinstance(typ, Option)
                    or (isinstance(typ, type) and issubclass(typ, Context))
                )
    ...
    def perms_dict(self, guild_id: Optional[int]):
        ...
        for (oid, type), perm in final.items():
            perms.append({"id": oid, "type": type.value, "permission": perm})
        return {"id": self.id, "permissions": perms}

    def add_perm(
        self,
        target: Union[pda.Role, pda.abc.User, pda.Object],
        perm: bool,
        guild_id: Optional[int] = ...,
        ...
        :type target: Union[pda.Role, PartialRole, pda.Member,
            pda.User, PartialMember, pda.Object]
        :param bool perm:
        ...
        :param guild_id:
            ...
            Must be specified if ``target`` is a :class:`~pda.Object` or
            a guildless :class:`~pda.User`.
        ...
        :param ApplicationCommandPermissionType type:
            ...
            Must be specified if ``target`` is a :class:`~pda.Object`.
        ...
        * ``add_perm(target, perm, guild_id)`` will infer the type, but
          manually set the guild ID (e.g. with a :class:`~pda.User` and
          not a :class:`~pda.Member`).
        * ``add_perm(pda.Object(id), perm, guild_id, type)`` will manually
        ...
        * ``add_perm(..., guild_id=None)`` will do any of the above but apply
        ...
        """
        if type is None:
            if isinstance(target, pda.Role):
                type = ApplicationCommandPermissionType.ROLE
            elif isinstance(target, pda.abc.User):
                type = ApplicationCommandPermissionType.USER
            else:
                raise ValueError("Must specify type if target is not a pda.py model")
        if guild_id is ...:
            if isinstance(target, (pda.Role, pda.Member)):
    ...
    async def invoke(self, ctx):
        ...
        logger.debug(
            "User %s running, in guild %s channel %s, command: %s",
            ctx.author.id,
            ctx.guild.id,
            ctx.channel.id,
    ...
    async def can_run(self, ctx):
        ...
        while parent is not None:
            if parent.cog is not None:
                if (
                    hasattr(parent.cog, "cog_check")
                    and parent.cog.cog_check not in cogs
                ):
        ...
        return all(await check(ctx) is not False for check in parents)
...
logger = logging.getLogger("pda.ext.slash")
...
class SlashBot(commands.Bot):
    ...
    :class:`pda.ext.commands.Bot` are as follows:
    ...
    :param bool resolve_not_fetch:
        If :const:`True` (the default), pda objects passed in arguments
        will be resolved from the slash commands API, not retrieved or fetched.
    :param bool fetch_if_not_get:
        If :const:`False` (the default), pda objects passed in arguments
    ...
    def __init__(self, *args, **kwargs):
        ...
        async def on_ready():
            ...
            self._connection.parsers.setdefault(
                "INTERACTION_CREATE",
                lambda data: (self._connection.dispatch("interaction_create", data)),
            )
            try:
    ...
    async def application_info(self):
        """Equivalent to :meth:`pda.Client.application_info`, but
    ...
    async def on_interaction_create(self, event: dict):
        if event["version"] != 1:
            raise RuntimeError(
                ...
                "https://github.com/Kenny2github/pda-ext-slash/issues/new"
            )
        cmd = pda.utils.get(self.slash, id=int(event["data"]["id"]))
        if cmd is None:
            warn(
        ...
        if cmd is None:
            warn(
                ...
                "falling back to name-only search",
                SlashWarning,
            )
            cmd = pda.utils.get(self.slash, name=event["data"]["name"])
        if cmd is None:
            ...
            for guild_id, guild in guilds.items():
                for name, kwargs in guild.items():
                    ...
                    task = asyncio.create_task(
                        self.process_command(name, guild_id, route, kwargs)
                    )
                    tasks.append(task)
    ...
    async def sync_cmds(self, state, todo, done, guild_id):
        ...
        for name in to_create:
            state["POST"].setdefault(guild_id, {})[name] = {
                "json": todo[name].to_dict(),
                "cmd": todo[name],
            }
        for name in to_update:
            cmd_dict = todo[name].to_dict()
            up_to_date = all(
                done[name].get(k, ...) == cmd_dict.get(k, ...)
                for k in {"name", "description", "options", "default_permission"}
            )
            if up_to_date:
